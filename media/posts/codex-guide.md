...
インストールが終わったら、対象のプロジェクトのディレクトリで`codex`と打つだけです。はじめて触るなら、手元のコードを読ませて説明させるところから入るのが早道です。初回はChatGPTでのサインインか、別のサインイン方法かを選びます。ChatGPTでサインインすると、契約しているプランの利用量と請求の扱いになります。APIキーを使う場合はAPIの料金です。
...
自分のマシンで動かす以上、Full accessに寄せるほど事故の代償が大きくなります。試す段階では既定のまま触らないのが無難です。
...
入れ替えの最中なので、GPT-5.6のSol・Terra・Lunaも引き続き選べます。チャット側で使えるのはこの5.6系のほうです。
...
推論の深さはLow・Medium・High・Extra high・Max・Ultraの6段階です。**最初に選ぶ深さはモデルごとに違い**、公式はSolならMedium、LunaならHigh、AstraならLightから始めることを勧めています。

呼び名も画面で違います。いちばん軽い段階はデスクトップアプリとウェブのWorkとIDE拡張ではLight、CLIではLowと表示されます。上の2つも役割が分かれていて、Maxは選んだモデルに同じ1つの仕事をより長く考えさせる段階、Ultraはサブエージェントに作業を分けて走らせる段階です。深くするほど時間もトークンも増えます。
...
手元のマシンを占有したくない作業は、クラウド側に投げられます。隔離した環境が割り当てられ、複数の作業を並行して進められる形です。ChatGPTのプランで走らせるクラウドのチャットはGPT-5.6 Solを使い、手元で動かすときより利用枠を多く消費することがあると公式に書かれています。
...
2026年10月14日にChatGPTとChatGPT WorkとCodexから引退します。OpenAI APIは対象外です。設定やスクリプトにモデル名を書いている場合は、その前の入れ替えが要ります。

### Codexはどこまで勝手に実行しますか？

答えは選んだ権限モード次第です。既定のAsk for approvalでは作業中のフォルダの中で動き、インターネットに出るときと範囲の外に出るときに確認を取ります。Full accessではどのファイルも承認なしで編集できる状態になります。

### Codex CLIとIDE拡張はどちらがいいですか？

選ぶ基準は作業の形です。CLIはターミナルの中で完結させたい場合や、`codex exec`でCIや繰り返しの処理に組み込む場合に向きます。IDE拡張は開いているファイルと選択範囲をそのまま渡し、差分をその場で確認したい場合に向きます。
