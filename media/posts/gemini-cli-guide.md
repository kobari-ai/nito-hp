...
会社のGemini Code AssistのStandardまたはEnterpriseライセンスを使っている場合、発表には「アクセスは変わらない」と書かれています。最新のGeminiモデルの提供も従来どおりです。有料のGeminiおよびGemini Enterprise Agent PlatformのAPIキー経由でも、Gemini CLIは引き続き使えます。
...
日本語の解説記事の多くは2025年に書かれていて、この記載を前提にしています。手順どおりに進めても無料では通らないのは、記事が間違っていたからではなく、その後に前提が変わったためです。判断の基準にするなら、READMEではなく[公式ドキュメント](https://www.geminicli.com/docs/get-started/authentication)と発表のほうを見てください。

## 開発自体は止まっていない

Gemini CLIのリポジトリは動き続けています。安定版は毎週火曜のUTC 20:00、プレビュー版は毎週火曜のUTC 23:59、ナイトリー版は毎日UTC 0:00に公開される形です。直近のリリースはv0.60.0で、2026年9月15日に出ています。
