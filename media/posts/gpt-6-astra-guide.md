...
OpenAIは2026年9月3日（米国時間）に新しい最上位モデル「GPT-6 Astra」を発表し、翌4日から有料プランへ順次展開を始めました。発表ページの冒頭は「世界で最も知的で、最もアラインメントの取れたモデル」で、力を入れているのは回答の賢さよりも**パソコンの画面を操作して作業を終わらせる力**です。
...
GPT-6 Astra には呼び名が2つあります。APIのモデルIDと ChatGPT Work・Codex での表示は「GPT-6 Astra」、**ChatGPTの通常チャットでの表示は「GPT-6 Pro」**です。OpenAIのヘルプは「GPT-6 Pro は GPT-6 Astra を基盤としている」と説明しています。社内で「Astraを試したい」と「GPT-6 Proが出た」が別々に話題になっても、同じものです。
...
発表ページは数学やコーディングより先に画面操作の話から始まっており、構成そのものが変化を表していました。
...
既存のテンプレートに沿ったスライド、構造の整った文書、Excelの分析、複数の資料を照合して数字の食い違いを見つける作業について、OpenAIは自社で最も優れたモデルだと書いています。事務作業の完遂率を測る AutomationBench は18.1%から41.4%に上がりました。裏返せば**6割の事務タスクはまだ最後まで終わらない**ので、人が確認する前提は変わりません。
...
3つの点を押さえておくと、数字に振り回されません。第一に、**伸びているのは手順を最後まで遂行する系の項目**で、知識寄りの GPQA は1.4ポイントしか動いていません。第二に、Humanity's Last Exam では Claude Fable 5.1 に負けており、OpenAI自身がその行を表に載せています。第三に、第三者機関 Artificial Analysis の総合指数では61.2で、Claude Fable 5.1 の65.7を下回ります。ARC-AGI-3 の99.9%は、OpenAIの応答API向けの実行環境で測った値で、汎用の実行環境では62.7%だったとARC Prize側が公表しています。

## ChatGPTのどのプランで使えるか

ここが最も問い合わせの多い部分です。OpenAIヘルプ「GPT-5.6 and GPT-6 Pro in ChatGPT」（2026年9月17日時点）の記載を表にしました。
...
Plusの扱いは発表ページとヘルプで表現が違います。発表ページは「Plusを含む有料プランへ展開」と書き、ヘルプは「Plusは Work と Codex で使える」と書いています。**Plusで通常チャットのモデル一覧を探しても出てこないのは正常**で、Work か Codex を開くと使えます。Pro 200ドルで週200回を使い切った場合は、ChatGPTが自動で GPT-5.6 Thinking（Medium）に切り替えました。
...
依頼するときは、完成形のファイル形式と用途、参照してほしい資料を先に伝えます。手元のファイルやアプリを操作させる作業はデスクトップアプリが関わるので、会社で許可された範囲だけを渡します。
...
ヘルプは「自動更新の通知で更新した直後でも、手動で更新を確認してほしい」と書いています。デスクトップアプリで Astra が出ない相談の多くはここで解決します。
...
短い要約や定型文で困っていないなら、すべてを Astra に切り替える理由はありません。**複数の資料を見比べる仕事や、何度も修正を頼んでいる仕事**から試すと違いが出ます。社内で比べるなら生成にかかった時間だけでなく、人が直した時間と使った枠を一緒に記録すると判断しやすくなります。

## 安全面で知っておくこと

Astra は OpenAI の Preparedness Framework でサイバーセキュリティ能力が初めて「Critical」に分類されたモデルです。未知の脆弱性を見つけて突く能力があるため、一般提供版は攻撃コードの作成のような依頼を拒否し、監視システムが作業を止めることがあります。ChatGPT や Codex で止まったときは操作の確認を求められ、API では処理が停止します。正当な業務でも止まる場合があると OpenAI 自身が書いていました。
...
Astra は書かれた推論を人が監視しにくくなったと OpenAI が明記している点も見ておく必要があります。性能が上がる一方で、なぜその結論になったかを確かめにくいため、重要な判断では成果物を人が検証する前提を外せません。

## AI検索での見え方は変わるのか

nito は ChatGPT や Google の AI検索で企業がどう引用されるかを実測して対策する会社で、モデルが GPT-6 になると自社の出方が変わるのかという質問を受けます。
...
2026年9月3日に一部の組織へ、9月4日から ChatGPT の有料プランと API に順次展開されました。段階展開なので、同じプランでも表示される時期に差があります。

### 無料プランで使えますか

使えません。無料と Go の既定モデルは GPT-5.6 Luna です。Plus は Work と Codex で、Pro・Business・Enterprise は通常チャットでも使えます。

### ChatGPT で使うのに追加料金はかかりますか

かかりません。既存の月額の中で、プランごとの利用枠まで使えます。枠を超えた分は追加クレジットで続けられます。

### GPT-6 Pro と GPT-6 Astra は別物ですか

同じモデルです。通常チャットでの表示名が GPT-6 Pro、Work・Codex・API での名前が GPT-6 Astra です。

### GPT-6 Astra Pro とは何ですか

発表ページで Pro・Business・Enterprise のユーザーが使えるとされている上位の推論オプションです。通常チャットの「GPT-6 Pro」がこれにあたります。

### 日本語の精度は上がりましたか

OpenAI は日本語に限った数値を公開していません。長文脈の検索精度や指示の解釈は全体として上がっていますが、日本語での効果は自社の作業で確かめる必要があります。

### GPT-5.6 Sol は使えなくなりますか

使えます。有料プランの Instant から Extra High は GPT-5.6 Sol が動いており、API でも Sol・Terra・Luna は提供されています。
